...
public sealed class SteamInventory : SteamSharedClass<SteamInventory> {
    ...
    static void InventoryUpdated(SteamInventoryFullUpdate_t x) {
    ...
    public static void LoadItemDefinitions() {
        ...
        _ = Internal.LoadItemDefinitions();
    }
    ...
    public static async Task<InventoryResult?> DeserializeAsync(byte[] data, int dataLength = -1) {
        ...
        try {
            ...
            if (!Internal.DeserializeResult(ref sresult, ptr, (uint)dataLength, false))
                return null;
    ...
    public static async Task<InventoryPurchaseResult?> StartPurchaseAsync(InventoryDef[] items) {
        ...
        return new InventoryPurchaseResult { Result = r.Value.Result, OrderID = r.Value.OrderID, TransID = r.Value.TransID };
    }
}
