...
public sealed class SteamApps : SteamSharedClass<SteamApps> {
    ...
    public static string CommandLine {
        get {
            _ = Internal.GetLaunchCommandLine(out var strVal);
            return strVal;
        }
    }
    ...
    public static IEnumerable<DlcInformation> DlcInformation() {
        ...
        for (var i = 0; i < Internal.GetDLCCount(); i++) {
            ...
            yield return new() { AppId = appid.Value, Name = strVal, Available = available };
        }
    }
    ...
    public static void MarkContentCorrupt(bool missingFilesOnly) {
        _ = Internal.MarkContentCorrupt(missingFilesOnly);
    }
    ...
    public static IEnumerable<DepotId> InstalledDepots(AppId appid = default) {
        ...
        for (var i = 0; i < count; i++) {
            yield return new() { Value = depots[i].Value };
        }
    }

    /// <summary>
    ///     Gets the install folder for a specific AppID.
    ///     This works even if the application is not installed, based on where the game would be installed with the default
    ...
    public static DownloadProgress DlcDownloadProgress(AppId appid) {
        ...
        return new() { BytesDownloaded = punBytesDownloaded, BytesTotal = punBytesTotal, Active = true };
    }
    ...
    public static async Task<FileDetails?> GetFileDetailsAsync(string filename) {
        ...
        return new FileDetails { SizeInBytes = r.Value.FileSize, Flags = r.Value.Flags, Sha1 = string.Join("", r.Value.FileSHA.Select(x => x.ToString("x"))) };
    }
