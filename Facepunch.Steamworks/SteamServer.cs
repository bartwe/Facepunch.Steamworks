...
/// </summary>
public sealed class SteamServer : SteamServerClass<SteamServer> {
    static readonly List<SteamClass> openInterfaces = new();
    static bool _dedicatedServer;
    static int _maxplayers;
    static int _botcount;
    static string _mapname;
    static string _modDir = "";
    static string _product = "";
    static string _gameDescription = "";
    static string _serverName = "";
    static bool _passworded;
    static string _gametags = "";
    ...
    /// <summary>
    ///     This will occur periodically if the Steam client is not connected, and has failed when retrying to establish a
    ///     connection (result, stilltrying)
    /// </summary>
    ...
    public static void Init(AppId appid, SteamServerInit init, bool asyncCallbacks = true) {
        if (IsValid)
            throw new("Calling SteamServer.Init but is already initialized");
        ...
        var secure = init.Secure ? 3 : 2;

        //
        // Get other interfaces
        //
        if (!SteamInternal.GameServer_Init(ipaddress, init.SteamPort, init.GamePort, init.QueryPort, secure, init.VersionString)) {
            throw new($"InitGameServer returned false ({ipaddress},{init.SteamPort},{init.GamePort},{init.QueryPort},{secure},\"{init.VersionString}\")");
        }
    ...
    public static void UpdatePlayer(SteamId steamid, string name, int score) {
        _ = Internal.BUpdateUserData(steamid, name, (uint)score);
    }
    ...
    public static void HandleIncomingPacket(IntPtr ptr, int size, uint address, ushort port) {
        _ = Internal.HandleIncomingPacket(ptr, size, address, port);
    }
