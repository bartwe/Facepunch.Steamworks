...
public sealed class SteamFriends : SteamClientClass<SteamFriends> {
    ...
    ///     You can then show these chats inline in the game. For example with a Blizzard style chat message system or the chat
    ///     system in Dota 2.
    ...
    public static bool ListenForFriendsMessages {
        ...
        set {
            ...
            _ = Internal.SetListenForFriendsMessages(value);
        }
    }
    ...
    internal void InstallEvents() {
        ...
        Dispatch.Install<GameLobbyJoinRequested_t>(x => OnGameLobbyJoinRequested?.Invoke(new(x.SteamIDLobby), x.SteamIDFriend));
        Dispatch.Install<FriendRichPresenceUpdate_t>(x => OnFriendRichPresenceUpdate?.Invoke(new(x.SteamIDFriend)));
    ...
    /// <summary>
    ///     Called when a chat message has been received in a Steam group chat that we are in. Associated Functions:
    ///     JoinClanChatRoom. (friend, msgtype, message)
    /// </summary>
    ...
    static void OnFriendChatMessage(GameConnectedFriendChatMsg_t data) {
    ...
    static void OnGameConnectedClanChatMessage(GameConnectedClanChatMsg_t data) {
    ...
    static IEnumerable<Friend> GetFriendsWithFlag(FriendFlags flag) {
        for (var i = 0; i < Internal.GetFriendCount((int)flag); i++) {
            yield return new(Internal.GetFriendByIndex(i, (int)flag));
        }
    }
    ...
    /// <summary>
    ///     Activates the Steam Overlay to open the invite dialog. Invitations sent from this dialog will be for the provided
    ///     lobby.
    /// </summary>
    ...
    ///     NOTE: It's a lot slower to download avatars and churns the local cache, so if you don't need avatars, don't request
    ///     them.
    ...
    ///     to your specified protocol (scheme) uris and instead dispatch a OverlayBrowserProtocolNavigation callback to your
    ///     game.
    /// </summary>
