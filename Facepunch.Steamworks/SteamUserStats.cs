...
public sealed class SteamUserStats : SteamClientClass<SteamUserStats> {
    ...
    public static IEnumerable<Achievement> Achievements {
        get {
            for (var i = 0; i < Internal.GetNumAchievements(); i++) {
                yield return new(Internal.GetAchievementName((uint)i));
            }
        }
    }
    ...
    internal static void InstallEvents() {
        ...
        Dispatch.Install<UserAchievementStored_t>(x => OnAchievementProgress?.Invoke(new(x.AchievementNameUTF8()), (int)x.CurProgress, (int)x.MaxProgress));
    ...
    ///     This call can be rate limited. Call frequency should be on the order of minutes, rather than seconds.You should
    ///     only be calling this during major state changes such as the end of a round, the map changing, or the user leaving a
    ///     server. This call is required to display the achievement unlock notification dialog though, so if you have called
    ///     SetAchievement then it's advisable to call this soon after that.
    ///     If you have stats or achievements that you have saved locally but haven't uploaded with this function when your
    ...
    /// <returns>
    ///     OK indicates success, InvalidState means you need to call RequestCurrentStats first, Fail means the remote
    ///     call failed
    /// </returns>
    public static async Task<Result> RequestGlobalStatsAsync(int days) {
        var result = await Internal.RequestGlobalStats(days);
        if (!result.HasValue)
            return Result.Fail;
    ...
    public static int GetStatInt(string name) {
        var data = 0;
        _ = Internal.GetStat(name, ref data);
        return data;
    }
