...
namespace Steamworks {
    ...
    /// </summary>
    public sealed class SteamNetworkingUtils : SteamSharedClass<SteamNetworkingUtils> {
        /// <summary>
        ///     So we can remember and provide a Get for DebugLEvel
        /// </summary>
        static NetDebugOutput _debugLevel;
        ...
        /// </summary>
        [MonoPInvokeCallback]
        static void OnDebugMessage(NetDebugOutput nType, IntPtr str) {
            debugMessages.Enqueue(new() { Type = nType, Msg = Helpers.MemoryToString(str) });
        }

        internal static void LogDebugMessage(NetDebugOutput type, string message) {
            debugMessages.Enqueue(new() { Type = type, Msg = message });
        }
