...
static unsafe class BufferManager {
    const int Bucket512 = 512;
    const int Bucket1Kb = 1 * 1024;
    const int Bucket4Kb = 4 * 1024;
    const int Bucket16Kb = 16 * 1024;
    const int Bucket64Kb = 64 * 1024;
    const int Bucket256Kb = 256 * 1024;

    static readonly Stack<ReferenceCounter> ReferenceCounterPool =
        new(1024);

    static readonly Dictionary<int, Stack<IntPtr>> BufferPools =
        new();

    static readonly Dictionary<IntPtr, ReferenceCounter> ReferenceCounters =
        new(1024);
...
#if DEBUG
        SteamNetworkingUtils.LogDebugMessage(
    ...
    static ReferenceCounter AllocateReferenceCounter(IntPtr ptr, int size, int referenceCount) {
    ...
    static void FreeReferenceCounter(ReferenceCounter counter) {
        if (counter == null)
    ...
    static void AllocateBuffer(int minimumSize, out IntPtr ptr, out int size) {
    ...
    static void FreeBuffer(IntPtr ptr, int size) {
    ...
    static int GetBucketSize(int size) {
        if (size <= Bucket512)
            return Bucket512;
        if (size <= Bucket1Kb)
            return Bucket1Kb;
    ...
    static int GetBucketLimit(int size) {
        if (size <= Bucket512)
            return 1024;
        if (size <= Bucket1Kb)
            return 512;
    ...
    delegate void FreeFn(NetMsg* msg);
}
