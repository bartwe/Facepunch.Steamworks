...
public class ConnectionManager {
    ...
    public void Close(bool linger = false, int reasonCode = 0, string debugString = "Closing Connection") {
        _ = Connection.Close(linger, reasonCode, debugString);
    }
    ...
    public unsafe int Receive(int bufferSize = 32, bool receiveToEnd = true) {
        ...
        var messageBuffer = stackalloc NetMsg*[bufferSize];

        while (true) {
            var processed = SteamNetworkingSockets.Internal.ReceiveMessagesOnConnection(Connection, new(&messageBuffer[0]), bufferSize);
