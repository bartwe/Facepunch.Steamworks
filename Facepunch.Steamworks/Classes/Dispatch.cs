...
public static class Dispatch {
        ...
        try {
            ...
            while (SteamAPI_ManualDispatch_GetNextCallback(pipe, ref msg)) {
                try {
                    ...
                    _ = SteamAPI_ManualDispatch_FreeLastCallback(pipe);
                }
            }
        }
        catch (Exception e) {
    ...
    /// </summary>
    static void ProcessCallback(CallbackMsg_t msg, bool isServer) {
    ...
    /// </summary>
    static void ProcessResult(CallbackMsg_t msg) {
        ...
        _ = ResultCallbacks.Remove(result.AsyncCall);
    ...
    internal static void OnCallComplete<T>(SteamAPICall_t call, Action continuation, bool server) where T : struct, ICallbackData {
        ResultCallbacks[call.Value] = new() { continuation = continuation, server = server };
    }
    ...
    internal static void Install<T>(Action<T> p, bool server = false) where T : struct, ICallbackData {
        ...
        list.Add(new() { action = x => p(x.ToType<T>()), server = server });
    }
    ...
    internal static void ShutdownServer() {
        ...
        foreach (var callback in Callbacks) {
            _ = Callbacks[callback.Key].RemoveAll(x => x.server);
        }
