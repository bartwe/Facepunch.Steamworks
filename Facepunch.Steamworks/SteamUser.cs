...
public sealed class SteamUser : SteamClientClass<SteamUser> {
    ...
    internal static void InstallEvents() {
        ...
        Dispatch.Install<DurationControl_t>(x => OnDurationControl?.Invoke(new() { _inner = x }));
    }
    ...
    ///     You can use this to add support for external site signups where you want to pop back into the browser after some
    ///     web page
    ...
    ///     DecompressVoice on the other side. The compressed data provided is in an arbitrary format and is not meant to be
    ///     played directly.
    ///     This should be called once per frame, and at worst no more than four times a second to keep the microphone input
    ///     delay as low as
    ...
    public static uint SampleRate {
        ...
        set {
            if (SampleRate < 11025)
                throw new("Sample Rate must be between 11025 and 48000");
            if (SampleRate > 48000)
                throw new("Sample Rate must be between 11025 and 48000");
    ...
    public static uint OptimalSampleRate => Internal.GetVoiceOptimalSampleRate();
		...
		{
			var from
 = Helpers.TakeBuffer( length );
			var to
 = Helpers.TakeBuffer( 1024 * 64 );

			//
			// Copy from input stream to a pinnable buffer
			//
			using ( var s
 = new System.IO.MemoryStream( from ) )
			{
				input.CopyTo( s );
			}
 ...
 = to )
    ...
    public static unsafe AuthTicket GetAuthSessionTicket() {
        ...
        fixed (byte* b = data) {
            ...
            return new() { Data = data.Take((int)ticketLength).ToArray(), Handle = ticket };
        }
    }
    ...
    public static async Task<AuthTicket> GetAuthSessionTicketAsync(double timeoutSeconds = 10.0f) {
        ...
        void f(GetAuthSessionTicketResponse_t t) {
            if ((ticket == null) || (t.AuthTicket != ticket.Handle))
                return;
            result = t.Result;
        }
    ...
    ///     As long as the in-game browser accepts and handles session cookies, Steam microtransaction checkout pages will
    ///     automatically recognize the user instead of presenting a login page.
    ///     NOTE: The URL has a very short lifetime to prevent history-snooping attacks, so you should only call this API when
    ///     you are about to launch the browser, or else immediately navigate to the result URL using a hidden browser window.
    ///     NOTE: The resulting authorization cookie has an expiration time of one day, so it would be a good idea to request
    ///     and visit a new auth URL every 12 hours.
    /// </summary>
    ...
    public static async Task<DurationControl> GetDurationControl() {
        ...
        return new() { _inner = response.Value };
    }
}
