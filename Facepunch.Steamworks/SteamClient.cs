...
namespace Steamworks {
    public static class SteamClient {
        ...
        public static void Init(uint appid, bool asyncCallbacks = true) {
            if (IsValid)
                throw new("Calling SteamClient.Init but is already initialized");

            Environment.SetEnvironmentVariable("SteamAppId", appid.ToString());
            Environment.SetEnvironmentVariable("SteamGameId", appid.ToString());

            if (!SteamAPI.Init()) {
                throw new("SteamApi_Init returned false. Steam isn't running, couldn't find Steam, AppId is ureleased, Don't own AppId.");
            }
            ...
            IsValid = openInterfaces.Count > 0;
        ...
        internal static void AddInterface<T>() where T : SteamClass, new() {
            var t = new T();
            var valid = t.InitializeInterface(false);
            if (valid) {
        ...
        internal static void ValidCheck() {
            if (!IsValid)
                throw new("SteamClient isn't initialized");
        }
    }
}
