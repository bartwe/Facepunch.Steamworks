...
public struct InventoryItem : IEquatable<InventoryItem> {
    ...
    ///     Once an item is removed it cannot be recovered.This is not for the faint of heart - if your game implements item
    ///     removal at all,
    ///     a high-friction UI confirmation process is highly recommended.ConsumeItem can be restricted to certain item
    ///     definitions or fully
    ///     blocked via the Steamworks website to minimize support/abuse issues such as the classic "my brother borrowed my
    ///     laptop and deleted all of my rare items".
    /// </summary>
    ...
    internal static Dictionary<string, string> GetProperties(SteamInventoryResult_t result, int index) {
        ...
        foreach (var propertyName in propNames.Split(',')) {
            strlen = Helpers.MemoryBufferSize;
    ...
    public DateTime Acquired {
        get {
            ...
            if (Properties.TryGetValue("acquired", out var str)) {
                ...
                return new(y, m, d, h, mn, s, DateTimeKind.Utc);
            }
