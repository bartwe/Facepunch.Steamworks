...
public struct ResultPage : IDisposable {
            ...
            for (uint i = 0; i < ResultCount; i++) {
                if (SteamUGC.Internal.GetQueryUGCResult(Handle, i, ref details)) {
                    ...
                    if (ReturnsKeyValueTags) {
                        ...
                        item.KeyValueTags = new((int)keyValueTagsCount);
                    ...
                    if (ReturnsAdditionalPreviews) {
                        ...
                        if (previewsCount > 0) {
                            ...
                            for (uint j = 0; j < previewsCount; j++) {
                                ...
                                if (SteamUGC.Internal.GetQueryUGCAdditionalPreview(Handle, i, j, out var previewUrlOrVideo, out var originalFileName, ref previewType)) {
                                    item.AdditionalPreviews[j] = new(previewUrlOrVideo, originalFileName, previewType);
                                }
                            }
                        }
                    }
    ...
    ulong GetStat(uint index, ItemStatistic stat) {
        ulong val = 0;
    ...
    public void Dispose() {
        if (Handle > 0) {
            _ = SteamUGC.Internal.ReleaseQueryUGCRequest(Handle);
            Handle = 0;
        }
    }
}
