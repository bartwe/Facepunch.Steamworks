...
public struct LobbyQuery {
    ...
    public LobbyQuery WithKeyValue(string key, string value) {
        if (string.IsNullOrEmpty(key))
            throw new ArgumentException("Key string provided for LobbyQuery filter is null or empty", nameof(key));

        if (key.Length > SteamMatchmaking.MaxLobbyKeyLength)
            throw new ArgumentException($"Key length is longer than {SteamMatchmaking.MaxLobbyKeyLength}", nameof(key));
    ...
    public async Task<Lobby[]> RequestAsync() {
        ...
        for (var i = 0; i < list.Value.LobbiesMatching; i++) {
            lobbies[i] = new() { Id = SteamMatchmaking.Internal.GetLobbyByIndex(i) };
        }
