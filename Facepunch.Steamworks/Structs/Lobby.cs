...
public struct Lobby {
    ...
    public IEnumerable<Friend> Members {
        get {
            for (var i = 0; i < MemberCount; i++) {
                yield return new(SteamMatchmaking.Internal.GetLobbyMemberByIndex(Id, i));
            }
        }
    }
    ...
    public bool SetData(string key, string value) {
        if (key.Length > 255)
            throw new ArgumentException("Key should be < 255 chars", nameof(key));
        if (value.Length > 8192)
            throw new ArgumentException("Value should be < 8192 chars", nameof(key));
    ...
    public void SetGameServer(string ip, ushort port) {
        ...
        SteamMatchmaking.Internal.SetLobbyGameServer(Id, add.IpToInt32(), port, new());
    }
